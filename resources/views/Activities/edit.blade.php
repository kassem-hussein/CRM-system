...
@section('content')
    <div class="flex gap-1 items-center p-4">
        ...
        <a class="history">{{__('messages.edit_activity')}}</a>
    </div>
    <form class="p-4" action="/activities/{{$activity->id}}" method="post">
        ...
        <div class="flex flex-col my-1">
            ...
            <select name="opportunity_id" id="opportunity">
                ...
                @foreach ($opportunities as $opportunity)
                    <option value="{{$opportunity->id}}" @selected($activity->opportunity_id == $opportunity->id)>{{$opportunity->name}}</option>
                @endforeach
            </select>
        ...
        <div class="flex flex-col my-1">
            ...
            <input type="text" id="description" value="{{$activity->description}}" name="description" placeholder="{{__('messages.description')}}"/>
        ...
        <div class="flex flex-col my-1">
            ...
            <input type="text" id="type" value="{{$activity->type}}" name="type" placeholder="{{__('messages.activity_type')}}"/>
            @if ($errors->has('type'))
        ...
        <div class="flex flex-col my-1">
            ...
            <input type="date" id="date" value="{{$activity->date}}" name="date" placeholder="{{__('messages.activity_date')}}"/>
            @if ($errors->has('date'))
