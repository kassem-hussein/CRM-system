...
@section('content')
    <div class="flex gap-1 items-center p-4">
        ...
        <a class="history">{{__('messages.add_lead')}}</a>
    </div>
    <form class="p-4" action="/leads" method="post">
        @csrf
        <div class="flex flex-col my-1">
            <label for="customer" required>{{__('messages.customer')}}</label>
            <select type="text" id="customer" value="{{old('name')}}" name="customer_id">
        ...
        <div class="flex flex-col my-1">
            ...
            <input type="text" id="source" value="{{old('lead_source')}}" name="lead_source" placeholder="{{__('messages.lead_source')}}"/>
        ...
        <div class="flex flex-col my-1">
            ...
            <select type="text" id="status" value="{{old('lead_staus')}}" name="lead_status">
                <option value="">{{__('messages.select_status')}}</option>
                <option value="new">{{__('messages.new')}}</option>
                <option value="contacted">{{__('messages.contacted')}}</option>
